...
    // 改為「初速度 + 重力」模型，讓到頂點後立即往下掉
    gravity: 5.4,       // cm/s^2
    jumpImpulse: -1.5,  // cm/s
    maxFallSpeed: 2.9,  // cm/s
...
    bird.angle = lerp(-0.55, 1.15, (t + 1) / 2);
  }
...
  function gameOver() {
    state = "gameover";
    bird.velocityY = Math.max(bird.velocityY, cmToPx(0.9));
    playGameOverSound();
  }
...
  function createEnvelopeGain(startTime, attack, decay, sustainValue, release, peakValue) {
    const gainNode = audioContext.createGain();
    gainNode.gain.setValueAtTime(0.0001, startTime);
    gainNode.gain.linearRampToValueAtTime(peakValue, startTime + attack);
    gainNode.gain.linearRampToValueAtTime(sustainValue, startTime + attack + decay);
...
    bgmNodes.push(oscA, oscB, gain, filter);
  }
